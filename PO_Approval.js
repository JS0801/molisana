...
define(['N/record', 'N/format', 'N/log'], (record, format, log) => {
    ...
    function verifyToken(po, token) {
        const stored = String(value(po, CONFIG.tokenField) || '');
        // Timestamp is an absolute expiry in milliseconds; random suffix is 256 bits.
        if (!/^\d{13}\.[a-f0-9]{64}$/.test(token) || token !== stored || Number(token.split('.')[0]) <= Date.now()) {
            fail('This link is invalid or has expired. Please contact Molisana for a new link.');
        }
    }
    function page(body) {
        ...
        </style></head><body><header><div class="brand"><img src="${esc(CONFIG.logoUrl)}" alt="Molisana Imports"><div class="brand-label"><strong>Vendor Portal</strong>Purchase order confirmation</div></div></header><main>${body}</main><footer>Molisana Imports &bull; Vendor purchase order response</footer></body></html>`;
    }
    ...
    function onRequest(context) {
        ...
        try {
            ...
            verifyToken(po, token);
            const current = String(value(po, CONFIG.decisionField) || '');
            if ([CONFIG.acceptedValue, CONFIG.rejectedValue].includes(current)) {
                response.write(statusPage('Response already received', `This purchase order has already been ${current === CONFIG.acceptedValue ? 'accepted' : 'rejected'}. No changes have been made. Please contact Molisana if a correction is needed.`, value(po,'tranid'), true));
                return;
            }
