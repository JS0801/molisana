...
function (ui, file, log, search, runtime, crypto) {
  ...
  function generateCsvFile(isCron) {
    ...
    if (!rowsAssembly || rowsAssembly.length === 0) {
      throw new Error('Assembly file is empty or missing header.');
    }
    ...
    newHeaders.push('Expiry Status');

    var newContent = [];
    newContent.push(newHeaders);
    ...
    var fileName = isCron ? 'Item Avail Tool.csv' : 'Avail_Download_' + (new Date().getTime()) + '.csv';
  ...
  function onRequest(context) {
    ...
    html += '<div class="toolbar-wrap">' +
      ...
      '<img src="https://cdn-icons-png.flaticon.com/512/10630/10630240.png" alt="csv-icon" style="width:16px;vertical-align:middle;margin-right:6px;" />' +
      'Download CSV</button>' +
      '<div class="legend-wrap" id="colorLegendWrap">' +
        '<span class="legend-label">Color Legend:</span>' +
        '<span class="legend-pill legend-red" data-row-filter="warning"><span class="legend-dot"></span><span>Less than 2 Months</span></span>' +
        '<span class="legend-pill legend-purple" data-row-filter="expired"><span class="legend-dot"></span><span>Expired</span></span>' +
        '<span class="legend-pill legend-yellow" data-row-filter="expiring"><span class="legend-dot"></span><span>Expiring</span></span>' +
      '</div>' +
    '</div>';
    ...
    result.headersClean.forEach(function (hVal, idx) {
      ...
      html += '<th data-col-idx="' + idx + '">' +
        '<span class="th-filter-wrap">' +
          '<span class="th-label">' + escHtml(label) + '</span>' +
      ...
      'document.addEventListener("DOMContentLoaded", function(){' +
        ...
        'if(exportBtn){' +
          'exportBtn.addEventListener("click", function(e){' +
            ...
            'if(url){window.open(url,"_blank");}else{alert("Download not available yet.");}' +
          '});' +
        '}' +
        ...
        'function rowMatchesLegend(row){' +
          'if(activeLegendFilters.size===0) return true;' +
          'var warningVal = String(row.getAttribute("data-warning")||"").toLowerCase();' +
        ...
        'function applyFilters(){' +
          ...
          'for (var r = 0; r < rows.length; r++) {' +
            ...
            '}' +
            'if (show && !rowMatchesLegend(row)) show = false;' +
        ...
        'function openFilterPanel(btn,colIdx){' +
          ...
          'if(oldPanel){' +
            'var same=(oldPanel.__ownerTH===th);' +
            'oldPanel.remove();' +
            'if(th) th.classList.remove("th-filter-active");' +
            'if(same) return;' +
          '}' +
          ...
          'list.addEventListener("change", function(e){' +
            ...
            'if(cb.checked) tempSel.add(key); else tempSel.delete(key);' +
          '});' +
          ...
          'panel.querySelector(".actions").addEventListener("click", function(e){' +
            ...
            '}' +
            'if(act==="deselectall"){' +
              'var cbs2=list.querySelectorAll("input[type=checkbox]");' +
              'for(var j=0;j<cbs2.length;j++){' +
                'var cb2=cbs2[j];' +
                'if(cb2.checked) cb2.checked=false;' +
                'var key2=cb2.closest(".row").dataset.key||"";' +
                'tempSel.delete(key2);' +
              '}' +
              'return;' +
            '}' +
            'if(act==="apply"){' +
              'var allow=new Set();' +
              'tempSel.forEach(function(k){allow.add(k);});' +
          ...
          'function positionPanel(){' +
            ...
            'var ph=panel.offsetHeight||0, pw=panel.offsetWidth||260;' +
            'var belowTop=rect.bottom+6, aboveTop=rect.top-ph-6;' +
          ...
          'setTimeout(function(){' +
            'document.addEventListener("click", function handler(e){' +
              ...
              'if(container) container.removeEventListener("scroll",onScrollResize);' +
